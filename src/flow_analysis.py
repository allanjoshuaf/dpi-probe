...
def parse_endpoint(value: str) -> tuple[str, int]:
    ...
    if value.startswith("["):
        ...
        if ":" in host:
            raise ValueError("IPv6 must use [ADDRESS]:PORT")
...
def _run_tshark(
    ...
    try:
        ...
        raise RuntimeError("tshark analysis exceeded 90 seconds; use a shorter capture window") from exc
...
def _endpoint_display_filter(endpoint_addresses: list[str], port: int) -> str:
    ...
    for address in endpoint_addresses:
        ...
        protocol = 'ipv6' if parsed.version == 6 else 'ip'
        address_filters.append(
            f"({protocol}.dst == {address} and tcp.dstport == {port}) or "
            f"({protocol}.src == {address} and tcp.srcport == {port})"
        )
    return f"({' or '.join(address_filters)})"
...
def extract_packets(pcap_path: str, endpoint_addresses: list[str], port: int) -> list[dict[str, Any]]:
    ...
    for line in rows:
        ...
        direction = (
            "client_to_server" if dst in endpoint_addresses and dst_port == port
            else "server_to_client" if src in endpoint_addresses and src_port == port
            else "unknown"
        )
...
def _exact_analysis_flag_counts(
    ...
    for name, field in fields.items():
        rows = _run_tshark(
            pcap_path,
            ["tcp.srcport", "tcp.dstport", "ip.src", "ip.dst", "ipv6.src", "ipv6.dst"],
            f"({base}) and {field}",
        ...
        for line in rows:
            ...
            src = values[2].split(',', 1)[0] or values[4].split(',', 1)[0]
            dst = values[3].split(',', 1)[0] or values[5].split(',', 1)[0]
            direction = ("client_to_server" if dst in endpoint_addresses and dst_port == port
                         else "server_to_client" if src in endpoint_addresses and src_port == port else None)
            if direction:
...
def summarize_streams(packets: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for stream_id, rows in streams.items():
        ...
        def acknowledged(payloads, direction):
            if not payloads:
                return False
            first = payloads[0]
            end = (first["seq"] + first["payload_len"]) % 2**32
            return any(row["direction"] == direction and row["ack_flag"] and not row["rst"]
                       and row["frame"] > first["frame"] and (row["ack"] - end) % 2**32 < 2**31
                       for row in rows)
        first_client_payload_acknowledged = acknowledged(client_payload_packets, "server_to_client")
        first_server_payload_acknowledged = acknowledged(server_payload_packets, "client_to_server")
        ...
        if resets and reset_directions == {"client_to_server"}:
            ...
            and all(any(r["fin"] and r["direction"] == direction and r["frame"] < resets[0]["frame"] for r in rows)
                    for direction in ("client_to_server", "server_to_client"))
        ):
        ...
        elif client_payload and not server_payload:
            phase = "client_data_acknowledged_no_return_payload" if first_client_payload_acknowledged else "client_data_unacknowledged"
...
def compare_vantages(client_pcap: str, server_pcap: str, endpoint: str) -> dict[str, Any]:
    ...
    # Match streams by shared absolute sequence evidence, never by local stream
    # IDs or ports (NAT can change both). Ambiguous matches remain inconclusive.
    def groups(packets):
        result = collections.defaultdict(list)
        for packet in packets:
            result[packet["stream"]].append(packet)
        return result

    clients, servers = groups(client_packets), groups(server_packets)
    signatures = {sid: {_packet_signature(p) for p in rows if _important(p)} for sid, rows in servers.items()}
    candidates = {}
    for cid, rows in clients.items():
        client_signatures = {_packet_signature(p) for p in rows if _important(p)}
        candidates[cid] = [sid for sid, values in signatures.items() if client_signatures & values]
    pairs = [(cid, ids[0]) for cid, ids in candidates.items()
             if len(ids) == 1 and sum(ids[0] in other for other in candidates.values()) == 1]
    client_only, server_only = [], []
    for cid, sid in pairs:
        client_only.extend(_unmatched_ranges(clients[cid], servers[sid]))
        server_only.extend(_unmatched_ranges(servers[sid], clients[cid]))
    ...
    if not client_packets or not server_packets:
        assessment = "insufficient_capture_evidence"
    elif not pairs:
        assessment = "no_unambiguous_common_flow"
    elif client_only_server_rst:
    ...
    elif len(pairs) != len(clients) or len(pairs) != len(servers):
        assessment = "unmatched_flows_inconclusive"
    else:
    ...
    result = {
        ...
        "matched_flow_count": len(pairs),
        "unmatched_client_streams": [cid for cid in clients if cid not in {c for c, _ in pairs}],
        "unmatched_server_streams": [sid for sid in servers if sid not in {s for _, s in pairs}],
        ...
        "limitations": [
            ...
            "Only uniquely matched streams are compared. Payload coverage uses sequence ranges, not content authentication; capture boundaries can still mimic loss.",
        ],
    }
    return result


def _unmatched_ranges(source: list[dict[str, Any]], other: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Compare byte coverage within one paired flow despite GRO/TSO segmentation."""
    signatures = {_packet_signature(p) for p in other if _important(p)}
    missing = []
    for packet in source:
        if not _important(packet) or _packet_signature(packet) in signatures:
            continue
        if packet["payload_len"] and not (packet["syn"] or packet["rst"] or packet["fin"]):
            # Signed serial-number offsets also handle a 32-bit sequence wrap.
            ranges = []
            for candidate in other:
                if candidate["direction"] == packet["direction"] and candidate["payload_len"]:
                    offset = ((candidate["seq"] - packet["seq"] + 2**31) % 2**32) - 2**31
                    ranges.append((offset, offset + candidate["payload_len"]))
            covered = 0
            for start, end in sorted(ranges):
                if start > covered:
                    break
                covered = max(covered, end)
            if covered >= packet["payload_len"]:
                continue
